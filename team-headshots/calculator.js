...
(function() {
  ...
  function sliderToMembers(s) {
    s = +s;
    return s <= 500
      ? Math.round(3 + (s / 500) * 47)
      : Math.round(50 + ((s - 500) / 500) * 150);
  }

  function membersToSlider(m) {
    m = +m;
    return m <= 50
      ? Math.round(((m - 3) / 47) * 500)
  ...
  function getRate(n) {
    ...
    if (n >= 7) return 170;
    if (n >= 5) return 180;
    return n >= 3 ? SDPH_CONFIG.baseRate : 0;
  }
  ...
  function openModal() {
    ...
    html += '<div class="sdph-sr"><span>Per Person Rate</span><span>
  ...
  document.getElementById('sdph-print-btn').addEventListener('click', function() {
    ...
    h.push('<title>Session Estimate' + CL('title'));
    ...
    for (var i = 0; i < ds.length; i++) {
      h.push('<div style="display:flex;justify-content:space-between;font-size:13px;color:#555;' + (i < 2 ? 'margin-bottom:6px' : '') + '"><span>' + ds[i][0] + CL('span') + '<span style="font-weight:600;color:#1a1a1a">' + ds[i][1] + CL('span') + CL('div'));
    }
    h.push(CL('div'));
    ...
    for (var j = 0; j < pr.length; j++) {
      h.push('<tr style="border-bottom:1px solid #e8e8e8"><td style="padding:12px 18px;font-size:14px;color:#333">' + pr[j][0] + CL('td') + '<td style="padding:12px 18px;font-size:14px;font-weight:600;text-align:right">' + pr[j][1] + CL('td') + CL('tr'));
    }
 ...
 + d.r.toFixed(2) + '</span></div>';
    html += '<div class="sdph-sr"><span>Headshots Total</span><span>' + fmt(d.ht) + '</span></div>';
    if (d.on) {
      html += '<div class="sdph-sr"><span>On-Site Setup Fee</span><span>
 ...
 + SDPH_CONFIG.onsiteFee + '</span></div>';
    }
    if (d.g > 0) {
